...
const openDispute = async (user, orderId, payload) => {
  if (user.role !== "CUSTOMER") {
    throw new AppError("Only customers can open disputes.", 403, "FORBIDDEN");
  }
...
const respondToDispute = async (user, orderId, payload) => {
  ...
  if (!dispute) {
    ...
    throw new AppError("Dispute not found.", 404, "ORDER_DISPUTE_NOT_FOUND");
  }
...
const resolveDispute = async (user, orderId, vendorId, payload) => {
  if (user.role !== "ADMIN") {
    throw new AppError("Only admins can resolve disputes.", 403, "FORBIDDEN");
  }
  ...
  const result = await prisma.$transaction(async (tx) => {
    if (
      ...
      const settlementReversal = await payoutService.applyRefundAgainstPayout(
        {
          orderId,
          vendorId,
          refundAmount,
      ...
      return {
        ...
        payout: settlementReversal.payout,
        order: updatedOrder,
        settlementReversal,
      };
    }
